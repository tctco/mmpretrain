...
@MODELS.register_module()
class AdaFaceClsHead(ArcFaceClsHead):
    def __init__(
        self,
        num_classes: int,
        in_channels: int,
        ...
        super(AdaFaceClsHead, self).__init__(
            num_classes, in_channels, loss=loss, init_cfg=init_cfg
        )
    ...
    def forward(self, feats, target: Optional[torch.Tensor] = None):
        with autocast(enabled=False):
            pre_logits = self.pre_logits(feats)
            norms = torch.norm(pre_logits, p=2, dim=1, keepdim=True)
            cosine = self.norm_product(pre_logits)
            if target is None:
            ...
            # g_angular
            m_arc = torch.zeros(
                target.size()[0], cosine.size()[1], device=cosine.device
            )
